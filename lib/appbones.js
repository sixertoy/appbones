...
(function () {
    ...
    AppBones = Base.extend({
        ...
        _createBoneFolder: function (parent, name) {
            try {
                var dest = path.join(parent, name);
                dest = path.resolve(this.options.dest, dest);
                if (!fse.exists(dest)) {
                    mkdirp.sync(dest);
                }
                return dest;
        ...
        /*
        _createBoneTemplate: function (parent, filepath, src) {
        ...
        */
        ...
        _parseBonesObject: function (parent, object) {
            ...
            Object.keys(object)
                .map(function (key) {
                    dest = $this._createBoneFolder(parent, key);
                    return $this._createBones(dest, object[key]);
                });
            return true;
        },
        ...
        _parseBonesFiles: function (parent, files) {
            ...
            files.map(function (filepath) {
                ...
                if (!$this._.isEmpty(content)) {
                    index = (filepath.indexOf('_') !== -1);
                    dest = path.join($this.options.dest, parent, filepath.substr(index));
                    // si le fichier existe
                    // mais qu'il y a pas d'overwrite
                    // on arrete le process
                    /*
                    if(fse.exists(dest) && !$this.options.overwrite){
                        return false;
                    }
                    // si le fichier existe
                    // on le supprime
                    ...
                    */
                    ...
                    fse.write(dest, content);
                    fse.commit(function () {
                        console.log('AppBones :: File ' + dest + ' have been written to disk');
                    });
                }
            });
            return true;
